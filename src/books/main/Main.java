...
public class Main {
	...
	public static ArrayList<Publishers> getAllPublishers() {
		ArrayList<Publishers> publishers = (ArrayList<Publishers>) session
				.createQuery("select p from Publishers p order by p.name").list();
		return publishers;
	}
	...
	public static ArrayList<Books> getAllBooksByPublisherName(String publisherName) {
		...
		Query q = session
				.createQuery(
						"select b from Books b join b.publishers p where p.name = :publisherName" + " order by b.title")
